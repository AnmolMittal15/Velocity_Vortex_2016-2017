...
public class RangeSensorTest extends OpMode {
    ...
    MRRangeSensor range = new MRRangeSensor("range", hardwareMap);
    byte[] values;
    @Override
    public void init() {
        telemetry.addLine("Code has been uploaded");
    }

    @Override
    public void loop() {
        values = range.getDistance();
        telemetry.addData("Ultra Sonic", values[0] & 0xFF);
        telemetry.addData("ODS", values[1] & 0xFF);
    }
